...
function Calendar(obj) {														// jucke
	...
	var mouseX = event.clientX;	// 이벤트 발생한 현재 x좌표 위치
	var mouseY = event.clientY;	// 이벤트 발생한 현재 y좌표 위치
...
function doClick() {															// 날자를 선택하였을 경우
	...
	if (cal_Day.length > 7) {												// 날자 값이있으면		
		target.previousSibling.value=cal_Day						// 값 설정
	}
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML = "<TABLE WIDTH=100 BORDER=0 CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:8pt;font-family:Tahoma;'>"
			...
			+ "<TR ALIGN=CENTER BGCOLOR=ThreedFace STYLE='color:#444444;font-weight:bold;'><TD>일</TD><TD>월</TD><TD>화</TD><TD>수</TD><TD>목</TD><TD>금</TD><TD>토</TD></TR>";
